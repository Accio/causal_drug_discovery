...
```{r fig.height=3, fig.width=5}
{
  ...
  plot(coeftab(lm(y ~ x, data=xydata),
               lm(y ~ 0 + x, data=xydata)), pars="x")
  abline(v=2, lty=2)
}
```

```{r model0, fig.height=3, fig.width=3}
set.seed(1887)
model0 <- dagify(y ~ x,
                 exposure = "x",
                 outcome = "y")
model0 <- tidy_dagitty(model0,
                       layout="linear")
ggplot(model0, aes(x = x, y = y, xend = xend, yend = yend)) +
      geom_dag_point() +
      geom_dag_edges_fan() +
      geom_dag_text(size=12, vjust=0.3) +
      theme_dag()
```
What happens if we switch target variable with the random variable? Well 

```{r}
ggplot(xydata, aes(x=y, y=x)) + geom_point() +
  geom_smooth(method = "lm", se=FALSE, color="black", formula = my.formula) +
  ggpmisc::stat_poly_eq(formula = my.formula,
                        eq.x.rhs = "y",
...
x <- rbern(N, prob=0.5)
y <- rnorm(N, mean=x*2)
xybid <- data.frame(x=x, y=y)
ggplot(xybid, aes(x=x, y=y)) + geom_point() +
  geom_smooth(method = "lm", se=FALSE, color="black", formula = my.formula) +
  ggpmisc::stat_poly_eq(formula = my.formula,
               eq.with.lhs = "italic(hat(y))~`=`~",
...
ggplot(model1, aes(x = x, y = y, xend = xend, yend = yend)) +
      geom_dag_point() +
      geom_dag_edges_link() +
      geom_dag_text(size=12, vjust=0.3) +
      theme_dag()
...
xz.formula <- z ~ x
zy.formula <- y ~ z
xyPlot <- ggplot(filter(xzydata, z==1), aes(x=x, y=y)) + geom_point() +
  geom_smooth(method = "lm", se=FALSE, color="black", formula = my.formula) +
