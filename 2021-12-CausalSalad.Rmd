...
```{r learningCausal, fig.height=4, fig.width=4}
set.seed(1887)
...
expLearnCausal <- ggplot(xydata, aes(x=x, y=y)) + geom_point() +
  ...
  geom_hline(yintercept=median(xydata$y), lty=4, col="darkgray") +
  geom_line(data=data.frame(x=xydata_lm_newx,
                            y=xydata_lm_newy),
            col="darkgray", lty=4) +
  geom_smooth(method="lm", formula = y~x) +
...
```{r chainCondition, fig.height=3, fig.width=4}
chainYvX <- lm(y~x, data=xzydata)
chainYvXcZ <- lm(y~x+z, data=xzydata)

{
  par(mar=c(3, 3, 1.5, 1.5), mgp=c(2, 0.8, 0))
  with(xzydata,
     plot(coeftab(lm(y ~ x),
                  lm(y ~ x + z)),
              pars="x", xlab="Coefficient of x"))
}
```
...
chainYzPlotCondX <- ggplot(xzydata, aes(x=z, y=y, color=x)) + geom_point() +
  geom_smooth(method = "lm", se=FALSE, color="darkgray", formula = my.formula) +
  ggpmisc::stat_poly_eq(formula = my.formula,
                        eq.x.rhs = "z",
               eq.with.lhs = "italic(hat(y))~`=`~",
...
modelForkPlot <- ggplot(modelFork, aes(x = x, y = y, xend = xend, yend = yend)) +
      geom_dag_point() +
      geom_dag_edges_link() +
      geom_dag_text(size=12, vjust=0.3) +
      theme_dag()
print(modelForkPlot)
```
```{r shortFork, fig.height=1.5, fig.width=3}
set.seed(1887)
shortFork <- dagify(x ~ z,
                    y ~ z,
                 coords = list(x = c(x=0, z=1, y=2),
                               y = c(x=-0.5, z=0, y=-0.5))) %>%
  tidy_dagitty(layout="auto")
shortForkPlot <- ggplot(shortFork,
                        aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_point() +
  geom_dag_edges_fan() +
  geom_dag_text(size=12, vjust=0.3, color="white") +
  ylim(-1, 0.5) + xlim(-.1, 2.1) +
  theme_dag()
print(shortForkPlot)
```
...
ggplot(modelCollider, aes(x = x, y = y, xend = xend, yend = yend)) +
      ...
      theme_dag()
```
```{r shortCollider, fig.height=1.5, fig.width=3}
set.seed(1887)
shortCollider <- dagify(z ~ x,
                z ~ y,
                 coords = list(x = c(x=0, z=1, y=2),
                               y = c(x=0.5, z=0, y=0.5))) %>%
  tidy_dagitty(layout="auto")
shortColliderPlot <- ggplot(shortCollider,
                        aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_point() +
  geom_dag_edges_fan() +
  geom_dag_text(size=12, vjust=0.3, color="white") +
  ylim(-0.5, 1.5) + xlim(-.1, 2.1) +
  theme_dag()
print(shortColliderPlot)
```
...
# Addtional plot for the neurogenesis study

Original publications:

* Creer, David J. et al. “Running Enhances Spatial Pattern Separation in Mice.” Proceedings of the National Academy of Sciences 107, no. 5 (February 2, 2010): 2367–72.
* Lazic Stanley E. “Using Causal Models to Distinguish between Neurogenesis-Dependent and -Independent Effects on behavior.” Journal of The Royal Society Interface 9, no. 70 (May 7, 2012): 907–17.

```{r neuroM1, fig.width=3, fig.height=1}
set.seed(1888)
neuroM1 <- dagify(N ~ T,
                         B ~ N,
                 coords = list(x=c(T=0, N=1, B=2),
                               y=c(T=0, N=0, B=0))) %>%
  tidy_dagitty(layout="linear")
neuroM1Plot <- ggplot(neuroM1, aes(x = x, y = y, xend = xend, yend = yend)) +
      geom_dag_point() +
      geom_dag_edges_link() +
      geom_dag_text(size=10, vjust=0.5) +
      theme_dag()
print(neuroM1Plot)
```

```{r neuroM2, fig.width=3, fig.height=1}
set.seed(1888)
neuroM2 <- dagify(N ~ T,
                         B ~ T,
                 coords = list(x=c(T=1, N=0, B=2),
                               y=c(T=0, N=-.5, B=-.5))) %>%
  tidy_dagitty(layout="linear")
neuroM2Plot <- ggplot(neuroM2, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_point() +
  geom_dag_edges_link() +
  ylim(-1, 0.5) + xlim(-.1, 2.1) +
  geom_dag_text(size=10, vjust=0.5) +
...
```{r mr, fig.height=3, fig.width=3}
set.seed(1888)
mrModel <- dagify(G ~ S,
                  D ~ G,
                  G ~ U,
                  D ~ U,
                 coords = list(x=c(S=0, G=1, D=2, U=1.5),
                               y=c(S=0, G=0, D=0, U=1))) %>%
  tidy_dagitty(layout="auto")
mrPlot <- ggplot(mrModel, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_point() +
  geom_dag_edges_link() +
  ylim(-1, 2) + xlim(-.1, 2.1) +
  geom_dag_text(size=12, vjust=0.5) +
  theme_dag()
print(mrPlot)
```

```{r mr_FGF21, fig.height=3, fig.width=3}
set.seed(1888)
mrFgf21Model <- dagify(FGF21 ~ SNPs,
                  MASH ~ FGF21,
                  FGF21 ~ U,
                  MASH ~ U,
                 coords = list(x=c(SNPs=0, FGF21=1, MASH=2, U=1.5),
                               y=c(SNPs=0, FGF21=0, MASH=0, U=1))) %>%
  tidy_dagitty(layout="auto")
mrFgf21Plot <- ggplot(mrFgf21Model, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_point() +
  geom_dag_edges_link() +
  ylim(-1, 2) + xlim(-.1, 2.1) +
  geom_dag_text(size=4, vjust=0.5) +
  theme_dag()
print(mrFgf21Plot)
```
